...
class BufCommands(commands.Cog, name="Buffalo Commands"):
    ...
    async def forecast(
        self, interaction: discord.Interaction, when: required_times = None
    ):
        ...
        if when:
            ...
            if (
                when.lower() == "today"
                or calendar.day_name[date.today().weekday()].lower() in when.lower()
            ):
            ...
            return await interaction.response.send_message(
                f"Here is a detailed forecast for `{when}`:\n" f"{forecast}"
            )
        else:
