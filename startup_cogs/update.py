...
class Updater(commands.Cog, command_attrs=dict(hidden=True)):
    ...
    async def validate_role_exists(self, ctx, validate_role, role):
        if not validate_role:
            validate_role = await self.get_role_by_id_or_name(ctx, role)
    ...
    async def get_and_replace_role(self, ctx, role):
        add_role = role.get("role", await self.validate_role_exists(ctx, None, role))
        if add_role:
    ...
    async def get_and_edit_role(self, ctx, role):
        edit_role = role.get('role', await self.validate_role_exists(ctx, None, role))
        if edit_role:
