    ...
    async def callback(self, interaction: Interaction) -> Any:
        ...
        if role in interaction.user.roles:
            await interaction.response.send_message(
                f"You already have the role {role.mention}", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            message + f"Added role {role.mention}", ephemeral=True
        )
...
def generate_region_embed(roles):
    ...
    embed.add_field(
        name="Buffalo Cities and Towns",
        value=construct_embed_values(
            roles,
            ["Nearby", "Just Visiting", "Lockport", "North Tonawanda", "Niagara Falls"],
            False,
        ),
        inline=False,
    )
    embed.add_field(
        name="Niagara County Cities",
        value=construct_embed_values(
    ...
    embed.add_field(
        name="Miscellaneous",
        value=construct_embed_values(roles, ["Nearby", "Just Visiting"], True)
