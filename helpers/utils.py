...
async def create_channel_embed(ctx: commands.Context) -> discord.Embed:
    ...
    embed = discord.Embed(title="Channel Directory")
    for category in ctx.guild.categories:
        if (
            category.id in [cat["id"] for cat in update["edit"]["categories"]]
            or category.name == "Archive"
        ):
            continue
        value = ""
...
async def sync_channel_permissions(ctx: commands.Context) -> None:
    for channel in ctx.guild.channels:
        if channel.type.name == "category":
            ...
            continue
        if (
            channel.name == "📝-welcome-rules"
            or channel.name == "📯-announcements"
            or channel.name == "🪪-bflo-roles"
        ):
...
async def edit_channels(ctx: commands.Context) -> None:
    for channel in update["edit"]["channels"]:
        edit_channel: discord.abc.GuildChannel = discord.utils.find(
            lambda c: c.id == channel.get("id")
            or c.name == channel.get("name")
            or c.name == channel.get("new_name"),
            ctx.guild.channels,
        )
        if not edit_channel:
        ...
        new_category = discord.utils.find(
            lambda c: c.name == channel.get("category"), ctx.guild.categories
        )
        new_topic = channel.get("topic", edit_channel.topic)
        sync_permissions = True
        if (
            edit_channel.category == new_category
            and edit_channel.name == new_name
            and (edit_channel.permissions_synced or not edit_channel.category)
        ):
            logger.info(
                f"Channel {edit_channel.name} has already been edited. Skipping.."
            )
            continue
        try:
            ...
            new_channel = await edit_channel.edit(
                name=new_name,
            ...
            logger.info(
                f"Successfully edited {new_channel.name} | Category - {new_channel.category}"
            )
...
async def create_staff_embed(ctx: commands.Context) -> discord.Embed:
    ...
    embed = discord.Embed(title="Staff Members")
    mod_roles = [
        ctx.guild.get_role(696073200987013241),
        ctx.guild.get_role(696078290619990026),
        ctx.guild.get_role(1021403746326036592),
        ctx.guild.get_role(1029376856945868810),
    ]
...
async def create_send_roleview(
    ctx: commands.Context, edit_channel: discord.abc.GuildChannel
) -> None:
...
async def edit_categories(ctx: commands.Context) -> None:
    for category in ctx.guild.categories:
        ...
        for overwrite in overwrites_list:
            ...
            overwrites.update({role: overwrite})
        ...
        logger.info(
            f"Overwrites constructed. Checking if {category.name} overwrites already set.."
        )
        if category.overwrites == overwrites:
            logger.info(
                f"Category overwrites for {category.name} already set correctly. Skipping.."
            )
            continue
        ...
        logger.info(
            f"Checking channels in category {category.name} to make sure they are synced.."
        )
        for channel in category.channels:
            ...
            if channel.permissions_synced:
                ...
                continue
            logger.info(
                f"Syncing {channel.name} permissions to category {category.name}.."
            )
            await channel.edit(sync_permissions=True)
...
async def clear_channel_overwrites(ctx: commands.Context) -> None:
    for channel in ctx.guild.channels:
        ...
        if channel.name in [role["name"] for role in update["create"]["roles"]]:
            logger.info(
                f"Overwrites for new channel {channel.name} should not be cleared."
            )
            continue
        ...
        logger.info(
            f"Overwrites removed from channel {channel.name}. Sleeping for a couple seconds.."
        )
        await asyncio.sleep(1)
...
async def copy_roles_from_server(copy_guild: discord.Guild, exclude_roles):
    ...
    not_excluded_roles = [
        ...
        and role.id not in [exclude_role["id"] for exclude_role in exclude_roles]
    ]
...
update = {
    "create": {
        "roles": [
            ...
            },
            {
                ...
                "name": "Buffalo",
                "colour": "#1a5ee6",
                "display_icon": 719580209799626812,
                "hoist": True,
            },
            {
                "id": None,
    ...
    "edit": {
        "roles": [
            ...
            {
                "id": 699934215684489306,
                "name": "Expat",
                "new_name": "Just Visiting",
                "colour": "#0",
                "permissions": permissions["regions"],
            },
            {
                "id": 699934108314501150,
                "name": "Northtowns",
                "permissions": permissions["regions"],
                "color": "#0",
            },
            {
                "id": 699934144817528843,
                "name": "Southtowns",
                "permissions": permissions["regions"],
                "color": "#0",
            },
            ...
            {
                "id": 696073200987013241,
                "name": "Admins",
                "permissions": permissions["mods"],
            },
            {
                "id": 696078290619990026,
                "name": "Mods",
                "permissions": permissions["mods"],
            },
            {
                "id": 699649445024759899,
                "name": "Server Booster",
                "permissions": permissions["elevated"],
            },
            {
                "id": 1028440757117386892,
                "name": "Real Ones",
                "permissions": permissions["elevated"],
            },
            {
                "id": 978421055297314856,
                "name": "VIP - Rochester Discord Admin",
                "permissions": permissions["elevated"],
            },
            {
                "id": 1007626715918503987,
                "name": "Regulars",
                "permissions": permissions["elevated"],
            },
            {
                "id": 1039255296775815328,
                "name": "Rochesterian VIP",
                "permissions": permissions["elevated"],
            },
            {
                "id": 1029377303920255037,
                "name": "FoWny",
                "permissions": permissions["elevated"],
            },
        ],
        "categories": [
            {
                "id": 1029388594147762198,
                "name": "Admin",
                "overwrites": [1021403746326036592],
            },
            {
                "id": 1037836509161717790,
                "name": "TOWNY MODS",
                "overwrites": [1021403746326036592, 1029376856945868810],
            },
            {
                "id": 1021399801222397984,
                "name": "Friends of WNY",
                "overwrites": [
                    1021403746326036592,
                    1029376856945868810,
            ...
            {
                "id": 1058585359551836212,
                "name": "Admin",
                "overwrites": [696073200987013241, 717539403752407061],
            },
            {
                "id": 696096508910633000,
                "name": "Mods",
                "overwrites": [
                    696073200987013241,
                    696078290619990026,
        ...
        "channels": [
            {
                "id": 696171963240022068,
                "name": "📝-welcome-rules",
                "category": "Informational",
                "position": 0,
            },
            {
                "id": 696075954363301898,
                "name": "📯-announcements",
                "category": "Informational",
                "position": 1,
            },
            {
                "id": 696452764427092081,
                "name": "❓-help-desk",
                "category": "Informational",
                "position": 2,
            },
            {
                "id": 699928543798493226,
                "name": "🦬-set-your-roles-first",
                "new_name": "get-started",
                "category": None,
                "position": 0,
            },
            {
                "id": 696071995984707684,
                "name": "💬-general",
                "category": "🦬| Buffalo",
                "position": 0,
            },
            {
                "id": 729031489244626954,
                "name": "introductions",
                "new_name": "👋-bflo-intros",
                "category": "🦬| Buffalo",
                "position": 1,
            },
            {
                "id": 719626310841925663,
                "name": "channel-select",
                "new_name": "🪪-bflo-roles",
                "category": "🦬| Buffalo",
            ...
            {
                "id": 696207813948473394,
                "name": "🎮-gaming",
                "category": "Interests and Hobbies",
                "position": 0,
            },
            {
                "id": 698249578595614800,
                "name": "🎨🏎-hobbies",
                "new_name": "🏎-car-talk",
                "category": "Interests and Hobbies",
                "topic": "Vroom-Vroom - Talk about cars here",
                "position": 1,
            },
            {
                "id": 698705467500920892,
                "name": "🎧-music-theater-arts",
                "new_name": "🎧🍿📚-entertainment",
                "category": "Interests and Hobbies",
                "topic": "Your one stop shop for music, movies, and books",
                "position": 2,
            },
            {
                "id": 698659695577137262,
                "name": "🖥-tech",
                "category": "Interests and Hobbies",
            ...
            {
                "id": 960855040413802596,
                "name": "🤷-off-topic",
                "category": "Archive",
                "position": 1,
            },
            {
                "id": 702544939300683796,
                "name": "🍿-movies-and-tv",
                "category": "Archive",
                "position": 2,
            },
            {
                "id": 1034491595262799992,
                "name": "📚-books",
                "category": "Archive",
                "position": 3,
            },
            {
                "id": 697659404497190943,
                "name": "🚣🏾-sports",
                "category": "Archive",
                "position": 4,
            },
            {
                "id": 1060316382039920670,
                "name": "🍺-alcoholic-beverages",
                "category": "Archive",
                "position": 5,
            },
            {
                "id": 696924737242136689,
                "name": "🎤-suggestions",
                "category": "Archive",
                "position": 6,
            },
            {
                "id": 696094626284568788,
                "name": "💵-buy-sell-jobs",
                "category": "Archive",
                "position": 7,
            },
            {
                "id": 969344166922448936,
                "name": "🤖-bot-spam",
                "category": "Archive",
                "position": 8,
            },
            {
                "id": 696206515211599912,
                "name": "🧻-politics",
                "category": "Archive",
                "position": 9,
            },
            {
                "id": 696081453183926283,
                "name": "🏒-sabres",
                "category": "Archive",
                "position": 10,
            },
...
async def get_by_id_or_name(
    ctx, identifier: Dict, element_type
) -> Union[discord.Role | discord.abc.GuildChannel]:
    element_map = {
    ...
    result = discord.utils.find(
        lambda r: r.id == identifier.get("id") or r.name == identifier.get("name"),
...
async def clear_role_perms(role):
    ...
    logger.info(
        f"Removed permissions from role {role.name}. Sleeping for a couple seconds"
    )
    await asyncio.sleep(1)
...
async def copy_overwrites_from_buf_server(
    ctx: commands.Context, buf_cat: discord.CategoryChannel
):
    channel_overrides = {
        ctx.guild.default_role: buf_cat.overwrites_for(buf_cat.guild.default_role)
    }
...
async def pick_random_role_and_assign(ctx, member: discord.Member):
    ...
    role = random.choice(region_delete_roles)
...
async def create_channels(channels: List[Dict[str, Any]], ctx):
    ...
    for channel in channels:
        ...
        if func:
            try:
                category = await get_by_id_or_name(
                    ctx, {"name": channel["category"]}, "category"
                )
                logger.info(f"Creating channel {channel['name']}..")
                new_channel = await func(
                    name=channel["name"],
                    category=category if category else None,
                    topic=channel.get("topic"),
                )
                logger.info(
                    f"New channel {new_channel.name} created with id - {new_channel.id} and "
                    f"type - {new_channel.type.name}. Sleeping.."
                )
                await asyncio.sleep(1)
            except Exception as e:
                logger.warning(
                    f"Cannot create {channel['type']} channel {channel['name']}: {e}"
                )
...
async def get_overwrites(name, ctx):
    ...
    overwrites = {}
    default_overwrite = discord.PermissionOverwrite(view_channel=True)
    if name == "Archive":
    ...
    for role in update["create"]["roles"] + update["edit"]["roles"][0:4]:
        ...
        if find_role:
            overwrites.update({find_role: default_overwrite})
    return overwrites
...
async def create_categories(ctx: commands.Context, categories: List[Dict[str, Any]]):
    ...
    for category in categories:
        ...
        if not name:
            logger.warning(
                f"Could not find a category name in category set - {category}"
            )
            continue

        # Check to make sure the category doesn't exist in ctx server.
        new_category = discord.utils.find(
            lambda c: c.name == name, ctx.guild.categories
        )
        if not new_category:
            ...
            logger.info(
                f"Found category {name} already in {ctx.guild.name} - skipping creation "
...
async def copy_categories_from_server(ctx, copy_cats: discord.Guild, exclude_cats):
    ...
    not_excluded_cats = [
        ...
        for cat in cats
        if cat.id not in [exclude_cat["id"] for exclude_cat in exclude_cats]
    ]
...
async def clear_role_permissions(ctx):
    for role in ctx.guild.roles:
        if role.is_bot_managed():
            ...
            continue
        if role.name in [
            create_role["name"] for create_role in update["create"]["roles"]
        ]:
            logger.info(f"{role.name} permissions should not be cleared. Skipping..")
            continue
...
async def edit_roles(ctx: commands.Context):
    for role in update["edit"]["roles"]:
        ...
        if not edit_role:
            logger.warning(
                f"Unable to find {role['name']} to edit - Does it exist or has it been deleted?"
            )
            continue
        name = role.get("new_name", edit_role.name)
        colour = (
            discord.Colour.from_str(role.get("colour"))
            if role.get("colour")
            else edit_role.color
        )
        if name.startswith("Buffalo"):
            name = "(LEGACY) " + name
            colour = discord.Colour.from_str("#0")
            await edit_role.edit(name=name, colour=colour, position=1)
            continue
        perms = role.get("permissions")
        logger.info(f"Editing role {edit_role.name}")
        try:
            await edit_role.edit(name=name, colour=colour, permissions=perms)
